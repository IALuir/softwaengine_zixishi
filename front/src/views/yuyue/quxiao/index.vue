...
function getList() {
  ...
  getYuyueList(tempTime).then(response => {
    ...
    loading.value = false;
    if(response.length){
      //0：未到签到时间 1：正常签到，正常签退 2:正常签到，未到签退时间 3:正常签到,未签退 4:正常签到,超时签退 5:超时签到,未到签退时间 6:超时签到，超时签退 7:超时签到，正常签退 8:未签到
      const stateList = ["未结束：未到签到时间","已结束：正常签到，正常签退","未结束：正常签到，未到签退时间","已结束：正常签到,未签退","已结束：正常签到,超时签退","未结束：超时签到,未到签退时间","已结束：超时签到，超时签退","已结束：超时签到，正常签退","已结束：未签到"]
