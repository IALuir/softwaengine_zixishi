...
@RestController
@RequestMapping("/system/carposition")
public class CarPositionController extends BaseController
{
     ...
     * 查询压铸设备管理列表
     */
    @PreAuthorize("@ss.hasPermi('system:carposition:query')")
    @GetMapping("/{deviceId}")
    ...
    @PreAuthorize("@ss.hasPermi('system:carposition:list')")
    @GetMapping("/list")
    ...
    @PreAuthorize("@ss.hasPermi('system:carposition:carnumidlist')")
    ...
    @PreAuthorize("@ss.hasPermi('system:carposition:addtemp')")
